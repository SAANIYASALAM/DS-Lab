...
int mulpoly(struct poly p1[], struct poly p2[], int t1, int t2)
{
    int k =0;
    for (int i=0; i<t1; i++)
    {
        for (int j=0;j<t2;j++)
        {
            p3[k].expo = p1[k].expo+p2[k].expo;
            p3[k].coeff = p1[k].coeff+p2[k].coeff;
            k++;
        }
    }
    return k;
...
void main()
{
    ...
    int t4 = mulpoly(p1, p2, t1, t2);
    printf("the nultiplied polynomial is:\n");
    displaypoly(p3 , t4);
